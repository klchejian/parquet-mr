...
import static org.apache.parquet.filter2.indexlevel.IndexFilter.canDrop;
...
//      "double_field", "float_field"));
    ...
    for (ColumnChunkMetaData column : ccmd) {
      ...
      if (indexEncodedColumns.contains(name)) {
        assertTrue("Column should be index encoded: " + name,
            column.getEncodings().contains(Encoding.INDEX));
        ...
        if (name.startsWith("fallback")) {
          assertTrue("Column should be have some index encoding: " + name,
              column.getEncodings().contains(Encoding.INDEX));
        } else {
          assertFalse("Column should have no index encoding: " + name,
              column.getEncodings().contains(Encoding.INDEX));
        }
      }
    }
